...
def _try_load_from_submodule(lang_name: cs.SupportedLanguage) -> LanguageLoader:
    ...
    python_bindings_path = submodule_path / cs.BINDINGS_DIR / cs.SupportedLanguage.PYTHON
        ...
        try:
            ...
            for attr_name in language_attrs:
                if hasattr(module, attr_name):
                    logger.debug(ls.LOADED_FROM_SUBMODULE, lang=lang_name, attr=attr_name)
...
def _create_locals_query(language: Language, lang_name: cs.SupportedLanguage) -> Query | None:
...
def _process_language(
    ...
    try:
        ...
        queries[lang_name] = _create_language_queries(language, parser, lang_config, lang_name)
