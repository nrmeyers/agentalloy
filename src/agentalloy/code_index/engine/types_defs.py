...
class CursorProtocol(Protocol):
    def execute(
        self,
        query: str,
        params: dict[str, PropertyValue] | Sequence[BatchParams] | BatchWrapper | None = None,
    ) -> None: ...
    def close(self) -> None: ...
    @property
...
# NOTE (agentalloy vendoring): the interactive-CLI types (AgentLoopUI,
# ORANGE_STYLE, *_LOOP_UI) were removed along with the prompt_toolkit
# dependency — the vendored engine has no interactive surface.
...
MCPResultType = (
    str | QueryResultDict | CodeSnippetResultDict | ListProjectsResult | DeleteProjectResult
)
...
NODE_SCHEMAS: tuple[NodeSchema, ...] = (
    ...
    NodeSchema(NodeLabel.PACKAGE, "{qualified_name: string, name: string, path: string}"),
