...
def _is_tool_available(tool_name: str) -> bool:
    ...
    try:
        subprocess.run([tool_name, "--version"], check=False, capture_output=True, timeout=2)
...
def _get_cached_stdlib_result(language: str, full_qualified_name: str) -> str | None:
    ...
    if cache_key in _CACHE_TIMESTAMPS and time.time() - _CACHE_TIMESTAMPS[cache_key] > _CACHE_TTL:
...
class StdlibExtractor:
    ...
    def _extract_python_stdlib_path(self, full_qualified_name: str) -> str:
        ...
        cached_result = _get_cached_stdlib_result(cs.SupportedLanguage.PYTHON, full_qualified_name)
    ...
    def _resolve_python_entity_module_path(self, parts: list[str], full_qualified_name: str) -> str:
        module_name = parts[0]
        entity_name = parts[-1]
        ...
        if len(parts) >= 3 and self.repo_path and self.project_name:
            try:
                ...
                module_path = cs.SEPARATOR_DOT.join(parts[:-1])
                _cache_stdlib_result(cs.SupportedLanguage.PYTHON, full_qualified_name, module_path)
                return module_path
        ...
        result = (
            cs.SEPARATOR_DOT.join(parts[:-1]) if entity_name[:1].isupper() else full_qualified_name
        )
    ...
    def _extract_js_stdlib_path(self, full_qualified_name: str) -> str:
        cached_result = _get_cached_stdlib_result(cs.SupportedLanguage.JS, full_qualified_name)
        if cached_result is not None:
            return cached_result
    ...
    def _resolve_js_entity_module_path(self, parts: list[str], full_qualified_name: str) -> str:
        module_name = parts[0]
        entity_name = parts[-1]
        ...
        if _is_tool_available("node"):
            try:
                ...
                if subprocess_result.returncode == 0:
                    ...
                    if data[cs.JSON_KEY_HAS_ENTITY] and data[cs.JSON_KEY_ENTITY_TYPE] in {
