...
def parse_cpp_base_classes(base_clause_node: Node, class_node: Node, module_qn: str) -> list[str]:
    parent_classes: list[str] = []
    base_type_nodes = (
...
def extract_java_superclass(
    ...
    for child in superclass_node.children:
        if child.type == cs.TS_TYPE_IDENTIFIER:
            if resolved := resolve_superclass_from_type_identifier(child, module_qn, resolve_to_qn):
                return [resolved]
    return []
...
def is_preceded_by_extends(child: Node, parent_node: Node) -> bool:
    ...
    return child_index > 0 and parent_node.children[child_index - 1].type == cs.TS_EXTENDS
