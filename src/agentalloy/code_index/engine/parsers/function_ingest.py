...
class FunctionIngestMixin:
    ...
    def _ingest_all_functions(
        ...
        for func_node in captures.get(cs.CAPTURE_FUNCTION, []):
            ...
            self._register_function(func_node, resolution, module_qn, language, lang_config)
    ...
    def _resolve_function_identity(
        ...
        return self._fallback_function_resolution(func_node, module_qn, language, lang_config)
    ...
    def _try_unified_fqn_resolution(
        ...
        is_exported = (
            cpp_utils.is_exported(func_node) if language == cs.SupportedLanguage.CPP else False
        )
    ...
    def _fallback_function_resolution(
        ...
        return self._resolve_generic_function(func_node, module_qn, language, lang_config)
    ...
    def _handle_cpp_out_of_class_method(self, func_node: Node, module_qn: str) -> bool:
        ...
        class_name_normalized = class_name.replace(cs.SEPARATOR_DOUBLE_COLON, cs.SEPARATOR_DOT)
    ...
    def _resolve_cpp_function(self, func_node: Node, module_qn: str) -> FunctionResolution | None:
        func_name = cpp_utils.extract_function_name(func_node)
        if not func_name:
            if func_node.type == cs.TS_CPP_LAMBDA_EXPRESSION:
                func_name = (
                    f"{cs.PREFIX_LAMBDA}{func_node.start_point[0]}_{func_node.start_point[1]}"
                )
            else:
                return None
    ...
    def _resolve_generic_function(
        ...
        func_qn = self._build_function_qn(func_node, module_qn, func_name, language, lang_config)
    ...
    def _build_function_qn(
        ...
        if language == cs.SupportedLanguage.RUST:
            return self._build_rust_function_qualified_name(func_node, module_qn, func_name)

        nested_qn = self._build_nested_qualified_name(func_node, module_qn, func_name, lang_config)
    ...
    def _register_function(
        ...
        logger.info(ls.FUNC_FOUND.format(name=resolution.name, qn=resolution.qualified_name))
    ...
    def _generate_anonymous_function_name(self, func_node: Node, module_qn: str) -> str:
        ...
        if parent and parent.type == cs.TS_PARENTHESIZED_EXPRESSION:
            ...
            if (
                ...
                func_type = (
                    cs.PREFIX_ARROW if func_node.type == cs.TS_ARROW_FUNCTION else cs.PREFIX_FUNC
                )
    ...
    def _collect_ancestor_path_parts(
        ...
        while current and current.type not in lang_config.module_node_types:
            result = self._process_ancestor_for_path(func_node, current, lang_config, skip_classes)
            if result is False:
                return None
    ...
    def _format_nested_qn(self, module_qn: str, path_parts: list[str], func_name: str) -> str:
        if path_parts:
