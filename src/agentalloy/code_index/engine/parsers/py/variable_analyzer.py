...
if TYPE_CHECKING:
    ...
    class _VariableAnalyzerDeps(Protocol):
        def _infer_type_from_expression(self, node: ASTNode, module_qn: str) -> str | None: ...
...
class PythonVariableAnalyzerMixin(_VarBase):
    ...
    def _process_untyped_parameter(
        ...
        if (
            ...
            or not (inferred_type := self._infer_type_from_parameter_name(param_name, module_qn))
        ):
            return
        local_var_types[param_name] = inferred_type
        logger.debug(lg.PY_PARAM_TYPE_INFERRED, param=param_name, type=inferred_type)

    def _process_typed_parameter(self, param: ASTNode, local_var_types: dict[str, str]) -> None:
        param_name_node = next((c for c in param.children if c.type == cs.TS_PY_IDENTIFIER), None)
    ...
    def _find_best_class_match(
        ...
        logger.debug(lg.PY_BEST_MATCH, param=param_name, match=best_match, score=highest_score)
        return best_match
    ...
    def _calculate_match_score(self, param_lower: str, class_lower: str) -> int:
        ...
        if class_lower in param_lower:
            return int(cs.PY_SCORE_CONTAINS_BASE * (len(class_lower) / len(param_lower)))
        return 0
    ...
    def _process_self_assignment(
        ...
        if not (
            ...
            and (attr_name := left_text.decode(cs.ENCODING_UTF8)).startswith(cs.PY_SELF_PREFIX)
            and (assigned_type := self._infer_type_from_expression(right_node, module_qn))
        ):
            return
    ...
    def _infer_method_return_element_type(self, var_name: str, module_qn: str) -> str | None:
        if cs.PY_VAR_PATTERN_ALL in var_name or var_name.endswith(cs.PY_VAR_SUFFIX_PLURAL):
