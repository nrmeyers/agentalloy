...
class PythonExpressionAnalyzerMixin(_ExprBase):
    ...
    def _infer_type_from_expression(self, node: Node, module_qn: str) -> str | None:
        if node.type == cs.TS_PY_CALL:
            ...
            if (
                ...
                return self._infer_method_call_return_type(method_call_text, module_qn, None)
    ...
    @recursion_guard(
        key_func=lambda self, method_call, module_qn, *_, **__: f"{module_qn}:{method_call}",
    ...
    def _get_method_return_type_from_ast(self, method_qn: str) -> str | None:
        ...
        result = (
            self._analyze_method_return_statements(method_node, method_qn) if method_node else None
        )
    ...
    def _infer_attribute_type(self, attribute_name: str, module_qn: str) -> str | None:
        ...
        class_name = (
            "".join(word.capitalize() for word in attribute_name.split(cs.CHAR_UNDERSCORE))
    ...
    def _try_infer_from_self_assignments(self, attribute_name: str, module_qn: str) -> str | None:
        try:
