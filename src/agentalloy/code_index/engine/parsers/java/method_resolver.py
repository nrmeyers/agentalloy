...
class JavaMethodResolverMixin:
    ...
    def _resolve_static_or_local_method(
        ...
        return next(
            (
                ...
                and qn.split(cs.CHAR_PAREN_OPEN)[0].endswith(f"{cs.SEPARATOR_DOT}{method_name}")
            ),
            None,
        )
    ...
    def _resolve_instance_method(
        ...
        if inherited_result := self._find_inherited_method(resolved_type, method_name, module_qn):
    ...
    def _search_method_in_class(self, class_qn: str, method_name: str) -> tuple[str, str] | None:
    ...
    def _find_method_return_type(self, class_qn: str, method_name: str) -> str | None:
        ...
        ctx = get_class_context_from_qn(class_qn, self.module_qn_to_file_path, self.ast_cache)
        if not ctx:
            return None
    ...
    def _heuristic_method_return_type(self, method_call: str) -> str | None:
        ...
        if cs.JAVA_CREATE_PATTERN in method_lower or cs.JAVA_NEW_PATTERN in method_lower:
            parts = method_call.split(cs.SEPARATOR_DOT)
            if len(parts) >= 2:
    ...
    def _do_resolve_java_method_call(
        ...
        if result:
            ...
            logger.debug(ls.JAVA_INSTANCE_NOT_FOUND, type=object_type, method=method_name)
        return result
