...
class JavaTypeResolverMixin:
    ...
    def _find_superclass_using_ast(
        ...
        for child in node.children:
            if result := self._find_superclass_using_ast(child, target_class_name, module_qn):
                return result
    ...
    def _find_interfaces_using_ast(
        ...
        for child in node.children:
            if result := self._find_interfaces_using_ast(child, target_class_name, module_qn):
                return result
    ...
    def _traverse_for_class_declarations(self, node: ASTNode, class_names: list[str]) -> None:
        match node.type:
            case cs.TS_CLASS_DECLARATION | cs.TS_INTERFACE_DECLARATION | cs.TS_ENUM_DECLARATION:
