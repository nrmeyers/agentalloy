...
class JavaVariableAnalyzerMixin:
    ...
    @abstractmethod
    def build_variable_type_map(self, scope_node: ASTNode, module_qn: str) -> dict[str, str]: ...
    ...
    def _process_variable_declarator(
        ...
        if value_node := declarator_node.child_by_field_name(cs.FIELD_VALUE):
            if inferred_type := self._infer_java_type_from_expression(value_node, module_qn):
                resolved_type = self._resolve_java_type_name(inferred_type, module_qn)
                local_var_types[var_name] = resolved_type
                logger.debug(ls.JAVA_LOCAL_VAR_INFERRED, name=var_name, type=resolved_type)
                return
        ...
        for child in body_node.children:
            if child.type == cs.TS_FIELD_DECLARATION:
                ...
                if field_info.get(cs.FIELD_NAME) and field_info.get(cs.FIELD_TYPE):
                    ...
                    this_field_ref = f"{cs.JAVA_KEYWORD_THIS}{cs.SEPARATOR_DOT}{field_name}"
                    ...
                    logger.debug(ls.JAVA_CLASS_FIELD, name=field_name, type=resolved_type)
    ...
    def _process_java_assignment(
        ...
        if inferred_type := self._infer_java_type_from_expression(right_node, module_qn):
            resolved_type = self._resolve_java_type_name(inferred_type, module_qn)
            local_var_types[var_name] = resolved_type
    ...
    def _process_enhanced_for_statement(
        ...
        if type_node and name_node:
            self._register_for_loop_variable(type_node, name_node, local_var_types, module_qn)
        else:
            self._extract_for_loop_variable_from_children(for_node, local_var_types, module_qn)
    ...
    def _register_for_loop_variable(
        ...
        if (var_type := safe_decode_text(type_node)) and (var_name := safe_decode_text(name_node)):
    ...
    def _infer_java_type_from_expression(self, expr_node: ASTNode, module_qn: str) -> str | None:
    ...
    def _infer_java_method_return_type(
        ...
        call_string = (
            f"{object_ref}{cs.SEPARATOR_DOT}{method_name}" if object_ref else str(method_name)
        )
