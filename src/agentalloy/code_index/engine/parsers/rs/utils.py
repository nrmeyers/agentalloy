    ...
    match node.type:
        case cs.TS_IDENTIFIER | cs.TS_TYPE_IDENTIFIER:
            if name := safe_decode_text(node):
                full_path = f"{base_path}{cs.SEPARATOR_DOUBLE_COLON}{name}" if base_path else name
...
def _process_scoped_use_list(node: Node, base_path: str, imports: dict[str, str]) -> None:
    new_base_path = ""
...
def build_module_path(
    ...
    while current and current.type != cs.TS_RS_SOURCE_FILE:
        match current.type:
            ...
            case _ if include_classes and class_node_types and current.type in class_node_types:
