...
class JsTsModuleSystemMixin:
    ...
    def _extract_require_module_name(self, declarator: ASTNode) -> str | None:
        ...
        if function_node.text is None or safe_decode_text(function_node) != cs.JS_REQUIRE_KEYWORD:
            return None
    ...
    def _process_destructured_child(self, child: ASTNode, module_name: str, module_qn: str) -> None:
    ...
    def _process_commonjs_import(
        ...
        try:
            ...
            logger.debug(ls.JS_COMMONJS_IMPORT_FAILED, imported_name=imported_name, error=e)
    ...
    def _ingest_commonjs_exports(
        ...
        for query_text in query_texts:
            try:
                captures = QueryCursor(Query(language_obj, query_text)).captures(root_node)
                ...
                try:
                    ...
                    if not export_names:
                        for export_function in export_functions:
                            if export_function:
                                if name_node := export_function.child_by_field_name(cs.FIELD_NAME):
                                    if name_node.text:
