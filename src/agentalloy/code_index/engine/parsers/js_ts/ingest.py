...
class JsTsIngestMixin(JsTsModuleSystemMixin):
    ...
    def _ingest_prototype_inheritance_links(
        ...
        try:
            self._process_prototype_inheritance_captures(language_obj, root_node, module_qn)
        except Exception as e:
            logger.debug(lg.JS_PROTOTYPE_INHERITANCE_FAILED, error=e)

    def _process_prototype_inheritance_captures(self, language_obj, root_node, module_qn):
        ...
        if child_classes and parent_classes:
            for child_node, parent_node in zip(child_classes, parent_classes):
                ...
                logger.debug(lg.JS_PROTOTYPE_INHERITANCE, child_qn=child_qn, parent_qn=parent_qn)
    ...
    def _register_object_method(
        ...
        logger.info(lg.JS_OBJECT_METHOD_FOUND, method_name=method_name, method_qn=method_qn)
        self.ingestor.ensure_node_batch(cs.NodeLabel.FUNCTION, method_props)
    ...
    def _ingest_assignment_arrow_functions(
        ...
        try:
            ...
            for query_text in [
                ...
                self._process_arrow_query(lang_query, query_text, root_node, module_qn, lang_config)
        except Exception as e:
    ...
    def _find_object_name_for_method(self, method_name_node: ASTNode) -> str | None:
        ...
        while current:
            if current.type == cs.TS_VARIABLE_DECLARATOR:
                ...
                if left_child and left_child.type == cs.TS_IDENTIFIER and left_child.text:
    ...
    def _build_assignment_arrow_function_qualified_name(
        ...
        path_parts = self._js_collect_ancestor_path_parts(current, lang_config, skip_types)
        return self._js_format_qualified_name(module_qn, path_parts, function_name)
    ...
    def _js_extract_ancestor_name(self, node: ASTNode, lang_config: LanguageSpec) -> str | None:
        naming_types = (
