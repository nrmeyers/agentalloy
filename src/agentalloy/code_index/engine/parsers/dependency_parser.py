...
class PyProjectTomlParser(DependencyParser):
    ...
    def parse(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            if project_deps := data.get(cs.DEP_KEY_PROJECT, {}).get(cs.DEP_KEY_DEPENDENCIES, []):
            ...
            optional_deps = data.get(cs.DEP_KEY_PROJECT, {}).get(cs.DEP_KEY_OPTIONAL_DEPS, {})
...
class RequirementsTxtParser(DependencyParser):
    ...
    def parse(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            logger.error(ls.DEP_PARSE_ERROR_REQUIREMENTS.format(path=file_path, error=e))
        return dependencies
...
class PackageJsonParser(DependencyParser):
    ...
    def parse(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            logger.error(ls.DEP_PARSE_ERROR_PACKAGE_JSON.format(path=file_path, error=e))
        return dependencies

    def _load_and_collect_deps(self, file_path: Path, dependencies: list[Dependency]) -> None:
        with open(file_path, encoding=cs.ENCODING_UTF8) as f:
            data = json.load(f)
        ...
        for key in (
            ...
            dependencies.extend(
                Dependency(dep_name, dep_spec) for dep_name, dep_spec in data.get(key, {}).items()
            )
    ...
    def parse(self, file_path: Path) -> list[Dependency]:
        ...
        try:
            ...
            for dep_name, dep_spec in deps.items():
                version = (
                    dep_spec if isinstance(dep_spec, str) else dep_spec.get(cs.DEP_KEY_VERSION, "")
                )
            ...
            with open(file_path, encoding=cs.ENCODING_UTF8) as f:
                ...
                for line in f:
                    ...
                    if line.startswith(cs.GOMOD_REQUIRE_BLOCK_START):
                        ...
                        in_require_block = False
                        continue
                    elif line.startswith(cs.GOMOD_REQUIRE_LINE_PREFIX) and not in_require_block:
                        parts = line.split()[1:]
                        if len(parts) >= 2:
                            dependencies.append(Dependency(parts[0], parts[1]))
                    elif in_require_block and line and not line.startswith(cs.GOMOD_COMMENT_PREFIX):
                        parts = line.split()
                        if len(parts) >= 2:
                            dep_name = parts[0]
                            version = parts[1]
