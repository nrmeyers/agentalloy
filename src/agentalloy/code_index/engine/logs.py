...
SEMANTIC_NOT_AVAILABLE = "Semantic search dependencies not available, skipping embedding generation"
...
EMBEDDING_STORE_RETRY = (
    "Embedding upsert failed (attempt {attempt}/{max_attempts}), retrying in {delay:.1f}s: {error}"
)
EMBEDDING_BATCH_STORED = "Stored batch of {count} embeddings"
EMBEDDING_BATCH_FAILED = "Failed to store embedding batch: {error}"
EMBEDDING_SEARCH_FAILED = "Failed to search embeddings: {error}"
EMBEDDING_RECONCILE_OK = "Embedding reconciliation: all {count} expected embeddings found"
EMBEDDING_RECONCILE_MISSING = (
    "Embedding reconciliation: {missing} of {expected} embeddings missing (IDs: {sample_ids})"
)
EMBEDDING_RECONCILE_FAILED = "Embedding reconciliation check failed: {error}"
QDRANT_DELETE_PROJECT = "Deleting {count} embeddings for project '{project}'"
QDRANT_DELETE_PROJECT_DONE = "Deleted embeddings for project '{project}'"
QDRANT_DELETE_PROJECT_FAILED = "Failed to delete embeddings for project '{project}': {error}"
...
PROTOBUF_NO_MESSAGE_CLASS = "No Protobuf message class found for label '{label}'. Skipping node."
PROTOBUF_NO_ONEOF_MAPPING = "No 'oneof' field mapping found for label '{label}'. Skipping node."
PROTOBUF_UNKNOWN_REL_TYPE = "Unknown relationship type '{rel_type}'. Setting to UNSPECIFIED."
PROTOBUF_INVALID_REL = "Invalid relationship: source_id={source_id}, target_id={target_id}"
PROTOBUF_FLUSH_SUCCESS = (
    "Successfully flushed {nodes} unique nodes and {rels} unique relationships to {path}"
)
...
NO_DOCS_VIOLATIONS_FOUND = "No-docs violations found (module docstrings or inline comments):"
NO_DOCS_ERROR = "  {error}"
...
MG_NODE_BUFFER_FLUSH = "Node buffer reached batch size ({size}). Performing incremental flush."
MG_REL_BUFFER_FLUSH = (
...
MG_MISSING_PROP = "Skipping {label} node missing required '{key}' property (keys: {prop_keys})"
MG_NODES_FLUSHED = "Flushed {flushed} of {total} buffered nodes."
MG_NODES_SKIPPED = "Skipped {count} buffered nodes due to missing identifiers or constraints."
...
MG_RELS_FLUSHED = "Flushed {total} relationships ({success} successful, {failed} failed)."
MG_FLUSH_START = "--- Flushing all pending writes to database... ---"
MG_FLUSH_COMPLETE = "--- Flushing complete. ---"
MG_PARALLEL_FLUSH_NODES = "Parallel flushing {count} label groups with {workers} workers"
MG_PARALLEL_FLUSH_RELS = "Parallel flushing {count} relationship groups with {workers} workers"
...
MG_NO_CONN_RELS = "No database connection for relationship group '{pattern}', skipping flush."
...
TOOL_FILE_EDIT_SURGICAL = "[FileEditor] Attempting surgical block replacement in: {path}"
...
TOOL_SHELL_ALREADY_TERMINATED = "Process already terminated when timeout kill was attempted."
...
EDITOR_NO_CHANGES_IDENTICAL = "No changes detected - target and replacement are identical"
...
PATH_NOT_IN_QUESTION = "Could not find original path in question for replacement: {path}"
...
CALL_NESTED_FOUND = (
    "Found nested call from {caller} to {call_name} (resolved as {callee_type}:{callee_qn})"
)
CALL_DIRECT_IMPORT = "Direct import resolved: {call_name} -> {qn}"
CALL_TYPE_INFERRED = (
    "Type-inferred object method resolved: {call_name} -> {method_qn} (via {obj}:{var_type})"
)
...
CALL_INSTANCE_ATTR = (
    "Instance-resolved self-attribute call: {call_name} -> {method_qn} (via {attr_ref}:{var_type})"
)
...
CALL_INSTANCE_QUALIFIED = (
    "Instance-resolved qualified call: {call_name} -> {method_qn} (via {class_name}:{var_type})"
)
CALL_INSTANCE_INHERITED = (
    "Instance-resolved inherited call: {call_name} -> {method_qn} (via {class_name}:{var_type})"
)
...
CALL_CHAINED_INHERITED = (
    "Resolved chained inherited call: {call_name} -> {method_qn} (via {obj_expr}:{obj_type})"
)
...
CALL_SUPER_UNRESOLVED = "Could not resolve super() call: {call_name} in parents of {class_qn}"
CALL_JAVA_RESOLVED = "Java method call resolved: {call_text} -> {method_qn}"
CALL_UNEXPECTED_PARENT = "Unexpected parent type for node {node}: {parent_type}. Skipping."
...
IMP_REEXPORT_PY_ALL_FILTERED = "  __all__ filter excluded {symbol} from re-exports in {module}"
CALL_REEXPORT_RESOLVED = "Re-export chain resolved: {call_name} via {hops} hop(s) -> {final_qn}"
CALL_REEXPORT_CYCLE = "Re-export cycle detected resolving {call_name}; aborting at {module}"
...
IMP_RUST_PUB_USE = "Rust pub use re-export: {name} -> {target} ({kind})"
...
JS_VAR_TYPE_MAP_BUILT = (
    "Built JS variable type map with {count} variables (found {declarator_count} declarators total)"
)
...
JS_NO_PATTERN_MATCHED = "No type inference pattern matched for value node type: {node_type}"
JS_METHOD_CALL_INVALID = "Method call {method_call} doesn't have 2 parts"
JS_CLASS_RESOLVE_FAILED = "Could not resolve class name {class_name} in module {module_qn}"
...
JS_RETURN_ANALYZED = "Analyzed return statements for {method_qn}, got type: {return_type}"
JS_METHOD_RETURN_ERROR = "Error inferring JS method return type for {method_call}: {error}"
...
LUA_TYPE_INFERENCE_RETURN = "Lua type inference: {class_name}:{method_name}() returns {class_qn}"
...
PY_TYPE_INFER_ATTEMPT = "Attempting to infer type for parameter '{param}' in module '{module}'"
...
JS_OBJECT_ARROW_FOUND = "  Found Object Arrow Function: {function_name} (qn: {function_qn})"
JS_ASSIGNMENT_ARROW_FOUND = "  Found Assignment Arrow Function: {function_name} (qn: {function_qn})"
JS_ASSIGNMENT_FUNC_EXPR_FOUND = (
    "  Found Assignment Function Expression: {function_name} (qn: {function_qn})"
)
JS_ASSIGNMENT_ARROW_QUERY_FAILED = "Failed to process assignment arrow functions query: {error}"
JS_ASSIGNMENT_ARROW_DETECT_FAILED = "Failed to detect assignment arrow functions: {error}"

# (H) JS/TS module system logs
JS_COMMONJS_DESTRUCTURE_FAILED = "Failed to process CommonJS destructuring pattern: {error}"
JS_MISSING_IMPORT_PATTERNS_FAILED = "Failed to detect missing import patterns: {error}"
JS_COMMONJS_VAR_DECLARATOR_FAILED = "Failed to process variable declarator for CommonJS: {error}"
