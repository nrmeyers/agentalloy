...
Given a MarkdownChunk's prose body and a CodeGraphStore, extract typed entity
edges alongside the existing GOVERNS path. Edge kinds:

- **REQUIRES**: "X requires Y", "X depends on Y" — dependency declarations
- **TOUCHES**: "editing X affects Y" — file-touch / breakage relationships
- **CONSTRAINTS**: "X must not touch Y", "X is prohibited" — constraints
- **COMMAND**: `backtick-enclosed CLI commands` — instructions for LLMs
- **STAKEHOLDER**: "Legal flagged X", "team Y requires Z" — human/people references

All entities are deterministic: regex-based, no LLM, no network. Targets are
resolved to symbol fqn via the graph (3-tier resolution: exact, file-boundary,
name-match). Unresolved targets are dropped (no "best guess").

Bounded by ``MAX_ENTITIES_PER_DOC`` and ``MAX_EDGES_PER_JOB`` (readable from
``settings`` with defaults below). Zero regression: does not interfere with
existing GOVERNS edges.

The ingestion pipeline (:mod:`agentalloy.code_index.ingest.pipeline`) runs this
as a parallel step alongside ``_index_decisions``.

The knowledge push module (:mod:`agentalloy.api.knowledge_push`) uses
``graph.typed_edges_for_fqn()`` to surface typed entities in the decision
manifest alongside GOVERNS decisions.
"""
...
class EntityEdge:
    ...
    Unlike ``CodeEdge``, this uses ``src_fqn`` and ``dst_fqn`` for clarity —
    the ``src`` is always a doc chunk anchor, the ``dst`` is the resolved target
    fqn (empty for standalone COMMAND/STAKEHOLDER edges).
    """

    kind: str  # REQUIRES | TOUCHES | CONSTRAINTS | COMMAND | STAKEHOLDER
    src_fqn: str  # markdown doc anchor, e.g. "docs/design/x/approach.md::design"
    dst_fqn: str  # resolved target fqn (empty for standalone edges)
    file_path: str  # doc file path
    span: str  # matched span text (truncated to 80 chars)
    resolution_tier: int  # 0 = standalone, 1 = exact match, 2 = file-boundary, 3 = name-match
...
def _resolve_entity_target(
    ...
    """Attempt to resolve a prose-mentioned candidate to a symbol fqn via the graph.

    Three-tier resolution:
    1. Exact match — ``candidate`` is a known symbol fqn
    2. File-boundary match — candidate looks like a file path; check symbols in that file
    3. Name-match — candidate is a short name; check symbols_by_name
    ...
    # Tier 2: if candidate looks like a file path, look for symbols in that file
    if "." in clean or "/" in clean:
        base_path = clean.rstrip("*/")
...
def _index_entity_edges(
    ...
    n_written = store.upsert_edges(code_edges)
    counts: dict[str, int] = {}
    for e in all_entities:
        counts[e.kind] = counts.get(e.kind, 0) + 1

    return EntityIndexResult(
        entities_written=n_written,
        entities_dropped=len(all_entities) - n_written,
