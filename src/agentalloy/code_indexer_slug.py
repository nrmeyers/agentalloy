"""Canonical repo-slug derivation for codebase-indexer queries.

This is a **faithful mirror** of codebase-indexer's
``app/services/slug.py`` (``parse_github_remote`` / ``canonical_slug_for_path``
/ ``derive_slug``) plus ``app/config.py:slugify_repo``. codebase-indexer is the
system of record: it stores each repo's index under the slug it derives, and
AgentAlloy must produce the *identical* string or its ``/search`` queries 404.

⚠️ Keep this in sync with codebase-indexer. The two implementations MUST agree
character-for-character. The canonical rule is:
...
def canonical_slug_for_path(local_path: Path) -> str | None:
    ...
    try:
        ...
        logger.debug("code_indexer_slug: git probe failed for %s — %s", path, exc)
        return None
