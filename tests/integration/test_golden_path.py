"""Golden path: compose/retrieve/inspect surfaces against a live embed runtime.

Exercises the full request flow in a single pass:
  - health endpoint
  - runtime diagnostics
...
Skipped unless the configured embed runtime (``RUNTIME_EMBED_BASE_URL``,
default ``http://localhost:47951``) is reachable with the configured model
loaded.

Run with:
    pytest -m integration tests/integration/test_golden_path.py -v
"""
...
_settings = Settings()
LM_BASE = _settings.runtime_embed_base_url
EMBED_MODEL = _settings.runtime_embedding_model
...
def _embed_model_responds(model: str) -> bool:
    """Probe the embed runtime via /v1/embeddings — some runners hide the
    model slot from /v1/models."""
    try:
        r = httpx.post(
