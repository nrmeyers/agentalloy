...
def _hermetic_container_flow(monkeypatch: pytest.MonkeyPatch):
    ...
    docstring of tests/container/test_container_e2e.py). Everything here is therefore
...
class TestEntrypointPrebuiltCorpusSeed:
    ...
    def test_seed_checks_volume_has_no_corpus(self):
        ...
        script = _build_entrypoint_script("")
        assert '[ ! -d "$APP_DIR/data/agentalloy.overgraph" ]' in script
    ...
    def test_pack_ingest_gated_on_not_seeded(self):
        ...
        assert '&& [ ! -d "$APP_DIR/data/agentalloy.overgraph" ]; then' in script
...
class TestBakedEntrypoint:
    ...
    def test_baked_entrypoint_matches_generated(self):
        ...
        baked_path = Path(__file__).resolve().parents[2] / "container" / "entrypoint.sh"
