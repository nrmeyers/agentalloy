...
def _plan_prose() -> str:
    ...
    return " ".join(_pack("sdd-plan-and-contracts.yaml")["raw_prose"].split())
...
def test_old_four_tag_example_removed() -> None:
    ...
    assert "`pytest`, `fastapi`, `duckdb`, `async`" not in _plan_prose()
...
def test_plan_exit_gate_has_three_enforcement_nodes() -> None:
    # Gates moved from the old monolithic design-and-planning file to plan.
    all_of = _pack("sdd-plan-and-contracts.yaml")["exit_gates"]["all_of"]
    keys: set[str] = set()
    for node in all_of:
...
def test_plan_approval_node_keys_on_plan_docs() -> None:
    # approval `since_name_glob` must cover plan artifacts, not the build
