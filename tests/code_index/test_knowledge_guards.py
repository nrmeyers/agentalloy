...
def test_index_decisions_takes_no_network_handle() -> None:
    ...
    # plain bool escape hatch (#527 --prune-decisions) — no embed client, no
    # HTTP client — so it cannot make a network/paid-LLM call.
    params = set(inspect.signature(pipeline._index_decisions).parameters)
    assert params == {"graph", "changed", "removed", "chunks", "prune_decisions"}
