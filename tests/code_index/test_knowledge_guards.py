...
def test_index_decisions_takes_no_network_handle() -> None:
    # The phase receives only a graph store + the markdown change sets + a
    # plain bool escape hatch (#527 --prune-decisions) + a plain set of changed
    # code FQNs (#527 D) — no embed client, no HTTP client — so it cannot make
    # a network/paid-LLM call. This is an exact-match allow-list on purpose:
    # adding a parameter must be a deliberate edit here, so a handle can never
    # arrive unnoticed. Anything added must stay inert data or a local store.
