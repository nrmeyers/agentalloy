...
| `cline` | `~/.cline/data/settings/providers.json` — `openai-compatible` provider entry + `lastUsedProvider` | The old repo-local `.cline/settings.json` was **inert** (Cline never read it; e2e-matrix finding) — rewired to Cline's real provider store, live-verified via the headless cline CLI. User-scoped → bare `/v1` surface; applies to both the VS Code extension and the CLI. `agentalloy add cline` now adopts the upstream from this same store (skips the proxy's own `agentalloy-proxy` entry; prefers `lastUsedProvider`, else the sole remaining non-proxy provider) and records it per-repo, per-harness in `.agentalloy/upstream`. Cline stores the API key as a literal, so adoption is auth-transparent (`key_env` unset) — right for keyless local upstreams; keyed cloud ones set `--key-env`. An upstream adopted here can never affect another harness's forwarding (the native passthrough surfaces read only their own harness's entry). |
