...
- Code default `_DEFAULT_KEEP_THRESHOLD = 0.05` (lm_assist.py:61) → **keep it; it is the gated-off /
  fail-open default.** Stage B still HITs (D4+D5) and still feeds `skill_granular_select` (§4.1), so
  diversity selection is restored even at 0.05 — the *filter* is just inert until measured.
