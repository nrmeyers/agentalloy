...
function DependencyRow({ name, value }: { name: string; value: unknown }) {
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-2">
      <div className="text-sm font-medium text-[var(--text-primary)]">{name}</div>
      <span className="text-sm text-[var(--text-secondary)]">{fmtUnknown(value)}</span>
    </div>
  );
}
...
function KeyValueList({ record }: { record: Record<string, unknown> }) {
  ...
  if (entries.length === 0) return <p className="text-sm text-[var(--text-tertiary)]">unknown</p>;
  return (
    <dl className="grid grid-cols-2 gap-x-4 gap-y-2">
      {entries.map(([key, value]) => (
        <div key={key}>
          <dt className="text-xs font-medium text-[var(--text-tertiary)] uppercase">{key}</dt>
          <dd className="text-sm text-[var(--text-primary)] break-all">{fmtUnknown(value)}</dd>
        </div>
      ))}
    </dl>
  );
    ...
    <div className="space-y-6 max-w-3xl">
        ...
        <Card>
          ...
          {dependencies && (
            <div className="mt-4 divide-y divide-[var(--border-subtle)]">
              <h3 className="text-sm font-semibold text-[var(--text-secondary)] pb-2">Dependencies</h3>
          ...
          {Object.keys(extraHealth).length > 0 && (
            <div className="mt-4">
              <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-2">Details</h3>
      ...
      {readiness.error ? (
        ...
        <Card>
          ...
          <div className="mt-4">
            <h3 className="text-sm font-semibold text-[var(--text-secondary)] mb-2">Warm-up Progress</h3>
            <KeyValueList
              record={
