...
- Dev: `pnpm install && pnpm dev` (serves on :5173; run the FastAPI service on :48950 in parallel).
- Build: `pnpm build` (`tsc && vite build` → `dist/`, served as static files by the FastAPI process).
- Proxy: the Vite dev server forwards `/api`, `/telemetry`, `/health`, `/readiness`, and `/diagnostics` to `http://localhost:48950`.
