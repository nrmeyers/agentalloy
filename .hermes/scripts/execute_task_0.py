...
def main():
    ...
    wire_harness_path = repo_root / "src" / "agentalloy" / "install" / "subcommands" / "wire_harness.py"
    ...
    try:
        ...
        continue_marker = find_line_number(original_content, 'continue special case (already has proxy, skip)')
...
'''
    ...
    if legacy_start == -1 or hooks_end == -1:
        print(f"Error: Could not find legacy section boundaries (start={legacy_start}, end={hooks_end})")
        sys.exit(1)
    ...
    print(f"   Original: {original_lines} bytes, New: {new_content.split(chr(10)).count(chr(10))} lines")
    ...
    if test_path.exists():
        ...
        for i, line in enumerate(lines):
            if 'TestClaudeCode' in line:
                ...
                for j in range(i, min(i+100, len(lines))):
                    if 'assert len(result["files_written"]) == 2' in lines[j]:
                        # Check if this is for claude-code
                        if any('claude-code' in lines[k].lower() for k in range(max(0, j-10), j)):
        ...
        if updated:
            ...
            print(f"⚠️  Could not find TestClaudeCode assertion to update")
    else:
